...
namespace Capa_Negocio
{
    ...
    public class CN_Cliente_Login
    {
        ...
        public string AccesoCliente(string correo, string hash, out string Resumen)
        {
            ...
            {
                string temp = Clientes.AccesoCliente(satinar.SatinarInput(correo), cifrado.Cifrado_Password(hash), out Resumen);

                if (Resumen.Equals("Bloquear"))
                {
                    Clientes.BloquearCliente(correo);
                }
                else if (Resumen.Length > 70)
                {
                    GenerarToken(correo);
                }
                return temp;
        ...
        public bool CambiarContra(string token, string conttra, out string mensaje)
        {
            ...
            return Clientes.CambiarContrasena(token, temp, out mensaje);
        ...
        {
            return Clientes.VerificarTokens(token);
        }
        ...
        {
            return Clientes.VerificarCorreo(correo);
        }
        private bool AlmacenarTokens(string correo, string token, DateTime fechaExpiracion)
        {
            return Clientes.AlmacenarTokens(correo, token, fechaExpiracion);
        }
